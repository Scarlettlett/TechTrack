...
def filter(bboxes, class_ids, scores, nms_iou_threshold):
    ...
    for bbox in bboxes:
        ...
        width = bbox[2]
        height = bbox[3]
        # x = int(bbox[0] - bbox[2]/2) # Top-left x
        # y = int(bbox[1] - bbox[3]/2)  # Top-left y
        # width = int(bbox[2])
        # height = int(bbox[3])
...
if __name__ == "__main__":
    ...
    filtered_bboxes, filtered_class_ids, filtered_scores = filter(bboxes, class_ids, scores, nms_iou_threshold=0.3)
    print('\nAefore filter:')
